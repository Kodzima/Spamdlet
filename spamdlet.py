...
threads = input('Введите количество потоков для спамма (по умолчанию 1): ')
if threads != '':
    threads = int(threads)
else:
    threads = 1
...
response = json.loads(response.content)
wall_section_ids = [item['id'] for item in response['data']]
headers = {
...
def main():
    ...
    while True:
        ...
        for results in img['results']:
            for wall_section_id in wall_section_ids:
                payload = {
                                'wall_id' : wall_id,
